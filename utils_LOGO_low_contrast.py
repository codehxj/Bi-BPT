...
class JointTransform2D:
    ...
    def __call__(self, image, mask):
        ...
        if self.color_jitter_params:
            ...
            # Decrease contrast
            contrast_factor = 0.5  # Decrease contrast factor
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # read mask image
        # mask = cv2.imread(os.path.join(self.output_path, image_filename[: -3] + "jpg"),0) cv2.IMREAD_GRAYSCALE
        # mask = cv2.imread(os.path.join(self.output_path, mask_filename[: -3] + "jpg"),0)
