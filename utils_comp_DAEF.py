...
def to_long_tensor(pic):
    # handle numpy array
    img = torch.from_numpy(np.array(pic, np.uint8))  # 将数组转换成张量
...
class JointTransform2D:
    ...
    def __call__(self, image, mask):
        ...
        image, mask = F.to_pil_image(image), F.to_pil_image(mask)  # 将张量或者ndarray转换成PIL Image
        ...
        if np.random.rand() < self.p_flip:
            image, mask = F.hflip(image), F.hflip(mask)  # 水平翻转
        ...
        if np.random.rand() < self.p_random_affine:
            ...
            image, mask = F.affine(image, *affine_params), F.affine(mask, *affine_params)  # 对保持图像中心不变的图像应用仿射变换

        # transforming to tensor
        image = F.to_tensor(image)  # 将PIL图像或ndarray转换为tensor
