...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1 = cv2.pyrUp(image, dstsize=(256,256))
        imglist.append(img1)
        # img2 = cv2.pyrUp(img1, dstsize=(512,512))
        # imglist.append(img2)
        # print(image.shape)
        # read mask image
        masklist = []
        #mask = cv2.imread(os.path.join(self.output_path, image_filename[: -3] + "jpg"),0)
        mask = cv2.imread(os.path.join(self.output_path, mask_filename[: -3] + "png"),0)
        mask = cv2.resize(mask, dsize=(128, 128))
        mask[mask<=127] = 0
        mask[mask>127] = 1
        masklist.append(mask)
        mask1 = cv2.pyrUp(mask, dstsize=(256, 256))
        masklist.append(mask1)
        # mask2 = cv2.pyrUp(mask1, dstsize=(512, 512))
        # masklist.append(mask2)
        # correct dimensions if needed
        #image, mask = correct_dims(image, mask)
        imglist1 = []
        masklist1 = []
        for i in range(2):
            image_correct, mask_correct = correct_dims(imglist[i], masklist[i])
            if self.joint_transform:
                image, mask = self.joint_transform(image_correct, mask_correct)
            imglist1.append(image)
        ...
        return imglist1, masklist1, image_filename


class ImageToImage2D_(Dataset):
    """
    Reads the images and applies the augmentation transform on them.
    Usage:
        1. If used without the unet.model.Model wrapper, an instance of this object should be passed to
